...
export class VertFragGeomShader : public Shader
{
public:
    VertFragGeomShader(const std::string& vertexPath, const std::string& fragmentPath, const std::string& geomPath)
    {
        // 1. retrieve the vertex/fragment source code from filePath
        std::string vertexCode = getShaderCode(vertexPath);
        std::string fragmentCode = getShaderCode(fragmentPath);
        std::string geomCode = getShaderCode(geomPath);

        // 2. compile shaders
        GLuint vertex = compileShader(GL_VERTEX_SHADER, vertexCode.c_str());
        GLuint fragment = compileShader(GL_FRAGMENT_SHADER, fragmentCode.c_str());
        GLuint geom = compileShader(GL_GEOMETRY_SHADER, geomCode.c_str());
...
export class MaterialShader : public VertFragGeomShader
{
public:
    MaterialShader() : VertFragGeomShader("shaders/material.vert", "shaders/phong.frag", "shaders/material.geom") {}
};

export class MillingShader : public ComputeShader
{
public:
    MillingShader() : ComputeShader("shaders/mill.comp") {}
};

export class DrillShader : public VertFragShader
{
public:
    DrillShader() : VertFragShader("shaders/drill.vert", "shaders/phong.frag") {}
};